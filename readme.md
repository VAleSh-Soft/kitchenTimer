...
Эта кнопка добавлена для удобства переключения режимов экрана. Короткий клик этой кнопкой переключает отображение на экране часов и таймеров, т.е. дублирует (точнее, заменяет) длинный клик кнопкой **Timer**. Если эта кнопка не нужна, можно закомментировать строку `#define USE_MODE_BUTTON` в файле **kitchenTimer.h**, что приведет к некоторому уменьшению кода.
